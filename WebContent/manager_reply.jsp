...
<body>
	...
	<%}
		...
		while(rst.next()){
			...
			<button type="button" onclick="window.location.href='delete_reply_check.jsp?UserNo=<%=userno%>&BlockNo=<%=blockNo%>&BlockName=<%=BlockName%>&PostNo=<%=postno%>&ReplyNo=<%=replyno%>'">删除</button>
			<hr>
			<%
		}
	}
